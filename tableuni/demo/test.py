...
def with_db(table, db_path):
    ...
    df = [('Dataset', 'Method', 'Metric', 'Score', 'FileName')]
    for i in range(row, table.shape[0]):
        for j in range(column, table.shape[1]):
            db.write('{}\t{}\t{}\t{}\t{}\n'.format(table.iloc[0, j], table.iloc[1, j], table.iloc[i, 0], table.iloc[i, j], filename))
...
if __name__ == "__main__":
    ...
    print(db)
    render_mpl_table(db, header=(1, 0), col_width=1.2)
    plt.show()